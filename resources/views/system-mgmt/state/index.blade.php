...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        ...
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('state.create') }}"><i class="fa fa-plus-circle"></i> Add new state</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
      ...
      <div class="row">
        <div class="col-sm-12">
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            <thead>
              <tr role="row">
                <th width="20%" class="sorting text-center" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="state: activate to sort column ascending">State Name</th>
            ...
            @foreach ($states as $state)
                <tr role="row" class="odd">
                  ...
                  <td class="text-center">{{ $state->countries_name }}</td>
                  <td class="text-center">
                    {{-- <form class="row" method="POST" action="{{ route('state.destroy', ['id' => $state->id]) }}" onsubmit = "return confirm('Are you sure?')"> --}}
                        ...
                        <a href= "{{ route('state.edit',$state->id) }}"><i class="fa fa-edit"></i></a>
                        <a href="{{ route('state.destroy',$state->id) }}"><i class="fa fa-trash ml-4" style="color:red" onclick = "return confirm('Are you sure?')"></i></a>
                    </form>
                  </td>
              </tr>
            @endforeach
            ...
            <tfoot>
              <tr>
                <th class="text-center" width="20%" rowspan="1" colspan="1">State Name</th>
                <th class="text-center" width="20%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="country: activate to sort column ascending">Country Name</th>
